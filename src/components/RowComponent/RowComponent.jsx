...
const RowComponent = ({ flag, data }) => {
  ...
  return (
    <div className={classes}>
      {fruitItems?.map((item) => {
        return (
          <div key={item.id} className="w-300 min-w-[300px] md:w-340 md:min-w-[340px] h-auto my-6 px-4 pb-2 rounded-lg bg-cardOverlay backdrop-blur-lg hover:drop-shadow-lg">
            ...
            <div className="flex flex-col gap-1 items-end">
              ...
              <p className="text-headingColor font-semibold text-lg">
                <span className="text-xs mr-1 text-red-500">$</span>{item.price}
              </p>
            </div>
          </div>
        );
